...
export default function ContactActions({ email, linkedin }: ContactActionsProps) {
  return (
    <div className="mt-10 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
      <a
        href={`mailto:${email}`}
        className="inline-flex min-h-[82px] min-w-[260px] items-center justify-center gap-4 rounded-sm bg-[#102f2f] px-10 text-[2rem] font-medium text-white transition-colors duration-200 hover:bg-[#0d2424]"
      >
        <span>Email me</span>
        <MailIcon />
      </a>
      ...
      <Link
        ...
        target="_blank"
        rel="noreferrer"
        className="inline-flex min-h-[82px] min-w-[320px] items-center justify-center gap-4 rounded-sm border border-black/70 bg-transparent px-10 text-[2rem] font-medium text-[#0d2d3b] transition-colors duration-200 hover:bg-black/5"
      >
